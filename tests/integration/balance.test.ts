...
describe('Balance Management', () => {
  ...
  describe('Get-Balance', () => {
    it('should return zero balance for new address', async () => {
      const balance = await marketplaceProcess.getMarketplaceBalance(TEST_SENDER);
    ...
    it('should return balance after deposit', async () => {
      ...
      // Check balance
      const balance = await marketplaceProcess.getMarketplaceBalance(TEST_SENDER);
      assert.strictEqual(balance, '5000000000', 'Balance should match deposit amount');
    });
  });

  describe('Deposit-ARIO (Credit-Notice)', () => {
    it('should accept ARIO deposits via Credit-Notice', async () => {
      const result = await marketplaceProcess.depositArio('10000000000', TEST_ARIO_PROCESS, TEST_SENDER);
      
      assert.strictEqual(result.Action, 'Credit-Notice-Processed', 'Should process deposit');
      
      // Verify balance increased
      const balance = await marketplaceProcess.getMarketplaceBalance(TEST_SENDER);
      assert.strictEqual(balance, '10000000000', 'Balance should reflect deposit');
    });

    it('should handle multiple deposits', async () => {
      await marketplaceProcess.depositArio('1000000000', TEST_ARIO_PROCESS, TEST_SENDER);
      await marketplaceProcess.depositArio('2000000000', TEST_ARIO_PROCESS, TEST_SENDER);
      
      const balance = await marketplaceProcess.getMarketplaceBalance(TEST_SENDER);
      assert.strictEqual(balance, '3000000000', 'Balance should be sum of deposits');
    });
  });
  ...
  describe('Withdraw-Ario', () => {
    it('should withdraw ARIO from marketplace balance', async () => {
      ...
      await marketplaceProcess.depositArio('10000000000', TEST_ARIO_PROCESS, TEST_SENDER);
      ...
      const balance = await marketplaceProcess.getMarketplaceBalance(TEST_SENDER);
      assert.strictEqual(balance, '5000000000', 'Balance should reflect withdrawal');
    });
  ...
  describe('Get-Paginated-Balances', () => {
    ...
    it('should return balances after deposits', async () => {
      ...
      const userBalance = data.items.find((b: any) => b.address === TEST_SENDER);
      assert(userBalance, 'Should find user balance');
      assert.strictEqual(userBalance.balance, '5000000000', 'Balance should match deposit');
    });
  });
});
