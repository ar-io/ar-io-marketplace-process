...
async function sendSlackNotification(
  issues: Array<{ antId: string } & AntIssue>,
): Promise<void> {
  if (!SLACK_WEBHOOK_URL) {
    console.log(
      kleur.yellow('SLACK_WEBHOOK_URL not set, skipping Slack notification'),
    );
    return;
  }
  ...
  const blocks: SlackBlock[] = [
    {
      ...
      text: {
        ...
        text:
          issues.length > 0
            ? '⚠️ Marketplace ANT Issues Detected'
  ...
  if (issues.length > 0) {
    ...
    const issuesByType = {
      ...
      notOwnedByMarketplace: issues.filter((i) => i.notOwnedByMarketplace)
        .length,
    ...
    blocks.push({
      ...
      fields: [
        {
          type: 'mrkdwn',
          text: `*Not in Registry:* ${issuesByType.notInAntRegistry}`,
        },
        { type: 'mrkdwn', text: `*Bad CU:* ${issuesByType.badCu}` },
        {
          type: 'mrkdwn',
          text: `*Not Owned by Marketplace:* ${issuesByType.notOwnedByMarketplace}`,
        },
        {
          type: 'mrkdwn',
          text: `*Escrow Issues:* ${issuesByType.escrowIssue}`,
        },
        {
          type: 'mrkdwn',
          text: `*Unknown Errors:* ${issuesByType.unknownError}`,
        },
      ],
    });
    ...
    for (const issue of issuesToShow) {
      ...
      if (issue.badCu)
        issueDetails.push(
          `Bad CU (${issue.badCu.cuName}: ${issue.badCu.status})`,
        );
      if (issue.notOwnedByMarketplace)
        issueDetails.push(
          `Not owned by marketplace (owner: <${SCAN_URL}/${issue.notOwnedByMarketplace.owner}|${issue.notOwnedByMarketplace.owner?.slice(0, 8)}...>)`,
        );
      if (issue.escrowIssue) issueDetails.push(`Escrow: ${issue.escrowIssue}`);
      if (issue.unknownError) issueDetails.push('Unknown error');
      if (issue.hasAssociatedArNSName === false)
        issueDetails.push('No associated ArNS name');
...
async function getAllOrders() {
  ...
  const orders = [];
  while (true) {
    const result = await marketplace.getPaginatedOrders({
      cursor,
...
async function getAllIntents() {
  ...
  const intents = [];
  while (true) {
    const result = await marketplace.getPaginatedIntents({
      cursor,
      limit: paginationLimit,
    });
...
async function getAllTransfersOutOffMarketplaceFromPruning() {
  ...
  const transferOuts = (maybeTransferOuts || []).filter(
    (msg) =>
      msg.Tags.find((tag: any) => tag.name === 'Action')?.value === 'Transfer',
  );
  return { info: infoNotice, transfers: transferOuts };
}

async function getANTInfo(
  antId: string,
): Promise<{ error: unknown | null; info: AoANTInfo | null; antId: string }> {
  try {
    const ant = ANT.init({
      process: new AOProcess({
        processId: antId,
...
async function checkCuStatus(antId: string): Promise<{
  ...
  const res = await fetch(`${aoTestnetCuUrl}/state/${antId}`, {
    method: 'HEAD',
    redirect: 'follow',
  });
...
async function observeMarketplaceState() {
  ...
  spinner.succeed(
    `Found ${transfers.length} transfers out of the marketplace in the last pruning`,
  );

  spinner.start('Fetching ANTs owned by marketplace...');
  const ants = await antRegistry.accessControlList({
    address: marketplaceProcessId,
  });
  spinner.succeed(
    `Found ${ants.Owned.length} ants owned by the marketplace process`,
  );
  ...
  const antsWithActiveMarketplaceOrders = new Set<string>(
    orders.map((order) => order.dominantToken),
  );
  const antsWithActiveMarketplaceIntents = new Set<string>(
  ...
  for (const order of orders) {
    ...
    if (!antRegistryOwnedAnts.has(order.dominantToken)) {
      antIssueMapping.set(order.dominantToken, {
        ...antIssueMapping.get(order.dominantToken),
        notInAntRegistry: true,
      });
      antIssueMapping.set(order.dominantToken, {
        ...antIssueMapping.get(order.dominantToken),
        isOrder: true,
  ...
  for (const intent of intents) {
    ...
    if (!antRegistryOwnedAnts.has(intent.antProcessId)) {
      antIssueMapping.set(intent.antProcessId, {
        ...antIssueMapping.get(intent.antProcessId),
        notInAntRegistry: true,
      });
      antIssueMapping.set(intent.antProcessId, {
        ...antIssueMapping.get(intent.antProcessId),
  ...
  const cuStatusResults = await Promise.all(
    Array.from(aggregatedAntIds).map((antId) =>
      concurrencyLimit(() => checkCuStatus(antId)),
    ),
  );
  spinner.succeed(`Checked CU status for ${aggregatedAntIds.size} ANTs`);
  const cuStatusResultsMap = new Map<
    string,
    {
      error: boolean;
      cuName: string;
  ...
  for (const result of cuStatusResults) {
    ...
    if (result.error) {
      ...
      antIssueMapping.set(result.antId, {
        ...antIssueMapping.get(result.antId),
        badCu: rest,
      });
    }
  ...
  const transferToIds = transfers.map(
    (transfer) =>
      transfer.Tags.find((tag: any) => tag.name === 'Recipient')?.value,
  );
  const registryOwnedAnts = Array.from(antRegistryOwnedAnts);
  const antsToCheckOwnershipOf = registryOwnedAnts.filter(
    (antId) =>
      !transferToIds.includes(antId) && !cuStatusResultsMap.get(antId)?.error,
  );

  spinner.start(
    `Checking ownership of ${antsToCheckOwnershipOf.length} ANTs...`,
  );
  const antInfoResults = await Promise.all(
    antsToCheckOwnershipOf.map((antId) =>
      concurrencyLimit(() => getANTInfo(antId)),
    ),
  );
  ...
  for (const result of antInfoResults) {
    ...
    if (result.info?.Owner !== marketplaceProcessId) {
      antIssueMapping.set(result.antId, {
        ...antIssueMapping.get(result.antId),
        notOwnedByMarketplace: { owner: result.info?.Owner },
      });
      ...
      if (antsWithActiveMarketplaceIntents.has(result.antId)) {
        ...
        antIssueMapping.set(result.antId, {
          ...antIssueMapping.get(result.antId),
          escrowIssue: 'intent active but no credit-notice to marketplace',
        });
      }
  ...
  if (antIdsWithIssues.length > 0) {
    spinner.start(
      `Looking up ArNS names for ${antIdsWithIssues.length} ANTs with issues...`,
    );
    try {
      const arnsRecords = await ario.getArNSRecords({
        filters: { processId: antIdsWithIssues },
      });
      ...
      for (const antId of antIdsWithIssues) {
        ...
        if (arnsName) {
          antIssueMapping.set(antId, {
            ...antIssueMapping.get(antId),
            hasAssociatedArNSName: true,
            arnsName,
          });
        } else {
          antIssueMapping.set(antId, {
            ...antIssueMapping.get(antId),
            hasAssociatedArNSName: false,
          });
        }
      }
      spinner.succeed(
        `Found ArNS names for ${processIdToArnsName.size} of ${antIdsWithIssues.length} ANTs`,
      );
    } catch (error) {
  ...
  const issues = Array.from(antIssueMapping.entries()).map(
    ([antId, issue]) => ({ antId, ...issue }),
  );
